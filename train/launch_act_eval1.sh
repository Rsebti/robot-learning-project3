...
#   STEPS=50000 bash train/launch_act_eval1.sh           # longer run
#   POLICY_REPO_ID=<other> bash train/launch_act_eval1.sh
set -euo pipefail
...
if [[ ! -x "$LEROBOT_TRAIN_BIN" ]]; then
  printf '[launch_act_eval1] ERROR: lerobot-train not found at %s\n' "$LEROBOT_TRAIN_BIN" >&2
  exit 1
fi
...
printf '[launch_act_eval1] dataset=%s\n' "$DATASET_REPO_ID"
printf '[launch_act_eval1] output=%s\n'  "$OUTPUT_DIR"
printf '[launch_act_eval1] wandb entity=%s project=%s run=%s\n' "$WANDB_ENTITY" "$WANDB_PROJECT" "$RUN_NAME"

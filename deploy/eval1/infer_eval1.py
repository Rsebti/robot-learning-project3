"""
Standalone inference for the EVAL1 SO101 PlaceCube deliverable
(squint-native-iso, tag eval1_final, ckpt md5 0fbbcecfee4e07f3e78c3cfe36560ff2).

EVAL1 = place ONE cube in the bowl. NO colour conditioning.
Contract verified from the checkpoint weights + the Eval1 env:
  - rgb   : wrist camera -> (1,16,16,3) uint8 (center-crop, 128 then 16)
  - state : (1,12) = [measured_qpos(6), controller_target_qpos(6)]   (NO goal one-hot)
  - action: (6,) in [-1,1], pd_joint_target_delta_pos, 10 Hz
This is infer.py adapted for Eval1: state 12 (not 18), 10 Hz (not 30),
delta caps [0.1*5,0.2], REST_QPOS wrist_roll = -pi/2. Robot driver byte-identical.
...
"""
import os
# Windows: force OpenCV to use DirectShow instead of Media Foundation.
# MSMF hangs on cap.set(CAP_PROP_FRAME_HEIGHT) inside lerobot 0.4.3's camera
# init. Setting this BEFORE importing cv2 makes OpenCV skip MSMF.
...
# Windows: lerobot 0.4.3 calls cv2.VideoCapture(int_index) without a backend
# preference. The default Windows backend (MSMF) hangs on this webcam.
# Subclass cv2.VideoCapture so any single-int call forces CAP_DSHOW while
# still passing isinstance() checks inside lerobot.
_OriginalVideoCapture = cv2.VideoCapture

_BAD_BACKENDS = {int(cv2.CAP_ANY), int(cv2.CAP_MSMF)}  # broken on this PC

class _DShowVideoCapture(_OriginalVideoCapture):
    def __init__(self, *args, **kwargs):
        # lerobot 0.4.3 calls cv2.VideoCapture(index, self.backend); on
        # Windows self.backend = CAP_MSMF which hangs/fails on this webcam.
        # Replace any auto/MSMF backend with CAP_DSHOW (verified working).
...
CALIBRATION_ID = "so101_follower"         # matches so101_follower.json sitting next to this script
...
# (arm 0.1 / gripper 0.2 at 10 Hz -> 0.0333 / 0.0667 at 30 Hz) so the
# per-second motion speed stays equivalent. The actuator-lag mismatch isn't
# compensated -- this is exploratory; teammate's note warns against 30 Hz.
...
def main():
    ...
    p.add_argument("--episode_steps", type=int, default=150, help="control steps per episode (Eval1 sim ~50 @ 10 Hz = 5s; default 150 = 15s real-world margin)")
    ...
    try:
        for ep in episodes():
            if args.n_episodes > 0:
                print(f"\n── Episode {ep + 1}/{args.n_episodes} (goal color {args.goal_color}) ──", flush=True)
            ...
            print(f"[home] target REST  (deg): {np.rad2deg(REST_QPOS).round(1).tolist()}", flush=True)
            print(f"[home] ramping to rest pose (up to 20s) ...", flush=True)
            ...
            err_deg = home_qpos_deg - np.rad2deg(REST_QPOS)
            print(f"[home] home error    (deg): {err_deg.round(1).tolist()}  max={np.max(np.abs(err_deg)):.1f}", flush=True)
            # Settle pause + countdown so you can see the arm is parked and prep the scene
            print(f"\n[home] HOMED. Rollout starts in 5s (Ctrl+C to abort) ...", flush=True)
